...
RSpec.describe User, type: :model do
  describe 'validations' do
    it 'requires full name to be present' do
      user = User.new(full_name: nil, email: 'a@a', password: '123456')
    ...
    it 'requires password to be present' do
      user = User.new(full_name: 'Amanuel', email: 'a@a', password: nil)
      expect(user).to_not be_valid
    end
  end
  ...
  describe 'associations' do
    it 'has many categories' do
      @user = User.new(full_name: 'Amanuel', email: 'a@a', password: '123456')
